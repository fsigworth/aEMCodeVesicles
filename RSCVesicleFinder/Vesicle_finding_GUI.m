...
amPars.thresh=.1;
amPars.var=0;
amPars.maxVar=1; % avoid divide by zero
amPars.width=0;
amPars.edge=.5;
amPars.denseFilt=.0015;  % A^-1
amPars.dense=.5;
amPars.fixE1MapCtr=0;
...
sav.fileIndex=0;
sav.findInMask=1;
% The array of booleans h.sav.roboFlags tells us the steps to
% take, starting with automask off:
% 1: fit
% 2. fit more
% 3. automask on
% 4. fit and track
% 5. fit more
sav.roboFlags=[0 0 1 1 1];
% State variables
h.sav=sav;
...
disp(['Reading ' num2str(h.sav.fileIndex) ' ' fileName]);
...
%     e1Ctr=bins(e1Upper);
   ...
   % if we have images in absolute scale (0 means no intensity) then we can set
   % h.sav.automaskPars.fixE1MapCtr=1 so the previous abs value can be
   % kept. Otherwise this parameter must be zero!
   if h.sav.automaskFixed && h.sav.automaskPars.fixE1MapCtr
        e1Ctr=h.e1CtrValue;
    else
        h.e1CtrValue=e1Ctr;
    end;
...
if ~h.sav.automaskFixed
    h.sav.automaskPars.maxVar=mxVal;
end;
...
h.varMap=varMap/h.sav.automaskPars.maxVar;
% fdisp([max(h.varMap(:)) h.sav.automaskPars.maxVar]);
end
