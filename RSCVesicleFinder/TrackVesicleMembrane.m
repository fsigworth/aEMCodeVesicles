...
minRadiusA=120;
maxRadiusA=h.sav.vesicleRadii(2)*1.2; % Allow limited growth of vesicle maximum size
...
m=h.filtImage;
n=size(m);
msk=Crop(meGetMask(mi,round(mi.imageSize/h.ds0),1:h.maskIndex),n);
m=m.*msk;
...
%   Pick up the vesicle x,y and radius in our image
    ...
    % skip nonexistent or small vesicles
    if ~mi.vesicle.ok(ind,1) || cr<minRadiusA/pixA || cr>maxRadiusA 
        continue
    end;
...
%    
    ...
    if ~maskOverlap && slopeSqVal<maxSlopeSq && closure<maxClosure && ~isnan(s1)...
            && s1>1/maxS1Ratio && s1<maxS1Ratio && mi1.vesicle.r(1)<maxRadiusA/mi1.pixA
