...
maxShiftFrac=0.3; %%%%%%%
initStepA=80;  % initial step is ~ membrane width
...
if size(pars.nTerms,2)<2 % only gave the first column which are radius
    pars.nTerms(:,2)=1+floor(pars.nTerms(:,1)/2); % number of amplitude terms
end;
...
p.pixA=pixA;
p.center=[ctr ctr]+fracLoc; % translation
p.mbn=vd;  % vesicle model
...
%         Scale down the previously fitted pars by stepFraction.
        p.simpSteps(1:oldnParsRound)=startingSteps(1:oldnParsRound)*stepFraction;
        if iRound==nRounds  % last round
            p.convCriterion=convCriterion/5;
            if pars.nTermsForExtraRound<=nTermsRound
                p.simpSteps=startingSteps;
                p.simpSteps(1)=p.simpSteps(1)*stepFraction; % reduce only the 1st term.
title('extra');
%                  disp(['Extra round simp steps: ' num2str(p.simpSteps)]);
            end;
        end;
...
for iRad=1:nIters  % radius fitting
    ...
    %         disp(abs(p.vr));
    %     end;
    if p.nTerms(1)>2 % make a soft error contribution by RELU
        boundsErr=max(0,1e3*sum( abs(p.vr(3:end))/p.vr(1) - p.rConstraints(3:end) ));
    end;
    %     if mod(iRad,5)==0
    %         p.displayOn=2;
    %     else
    ...
    if p.displayOn && mod(iRad,ceil(40/sum(p.simpMask)))==1 % show iterations
%  disp([iRad P1]);
    subplot(3,2,3);
    imags(vfit);
    title(num2str(p.simpMask));
...
imags(p.imgc-vfit);
    ...
    lsErr=res'*res;
    %               Simplex update
    [P1,t]=Simplex(boundsErr+lsErr);  % update the vesicle radius
    sds=std(t.simp,1);
    if any(isnan(sds)) || all(sds<p.convCriterion/p.pixA)
        break;
    end;
...
if ~any(isnan(vfit(:)))
    ...
    P1=Simplex('centroid');
    p.vr=Pack(P1,p.nTerms);
    [vFit,a,p.center]=LLSFit(p); % Get the translation one last time
    [vFit,s]=AmpFitNX(p);
    res=p.imgc(:)-vFit(:);
    lsErr=res'*res;
    t=p.center;
...
%    Add the displacement of the cc peak to p.center to give the new
%    translation t. Also compute a=[const vesAmp]'
%    the simple fit of a constant term and the amplitude of the general,
%    filtered vesicle with (non-updated) parameters {p.vr,p.mbn,p.center}.
%    Return the model with constant and amplitude applied (but translation
%    not applied.)
ndis=size(p.imgc,1);
ctr=floor(ndis/2+1);

v=-VesicleFromModelGeneral(ndis,p.vr,p.mbn,p.center);
...
t=p.center+[xi yi]-ctr;  % add the incremental translation
...
%         Linear fit of p.imgc with general model {p.vr,p.mbn,p.center} as filtered
...
vArray(:,:,1)=-VesicleFromModelGeneral(ndis,p.vr,p.mbn,p.center);
vAFilt=vArray;
exPos=p.ringPeaks;
exSigma=p.ringSigma;
Ndis=ndis;
if nt>1
    vArray(:,:,2:nt)=-p.pixA*VesicleFromRings(ndis,exPos,exSigma,p.vr,p.center);
end;
