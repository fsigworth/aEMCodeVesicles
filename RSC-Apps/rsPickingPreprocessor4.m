...
%
% The general algorithm is described in Liu Y, Sigworth FJ. Automatic
% cryo-EM particle selection for membrane proteins in spherical liposomes.
% J. Struct. Biol. 185:295-302, 2014. PMC3978669
% with additions (mostly in SimpleRSPicker) to test for excessive
% background after subtracting the template from the vicinity of a putative
% particle.
...
disp(['mapMode = ' pars.mapMode]);
KvAlphaMode=strcmpi(pars.mapMode,'KvAlpha');
nterms=26;

if KvAlphaMode
    pars.angleOversample=2;
    pars.outputImageSize=[1440 1024];%% Special for KvAlpha %%%%%%%
    nterms=51; %%%%%%
    nterms=101;
end;
...
switch pars.mapMode
    ...
    case 'KvAlpha'
        ppVals.membraneOffsetA = 22;  % membrane is 22 � above particle center for RSO particle
        %    Hence a particle center > r-22 implies an inside-out particle.
        %
        %     mapName='/Users/fred/Structures/kv1.2/KvAlphaMap.mrc';
        %     mapName='~/Structures/kv1.2/KvMap.mrc'; % weak membrane subtraction
        mapName=[mapPath 'KvAlphaMapMbnSub.mrc'];  % stronger membrane subtraction.
        dimShift=0;
        symmetry=4;
...
% new tighter values
ppVals.nAlpha=36*pars.angleOversample;
ppVals.nBeta=12*pars.angleOversample;  % even is best; number of betas per hemisphere.
ppVals.nGamma=24*pars.angleOversample/symmetry;
