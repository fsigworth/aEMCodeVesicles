...
% Now saves the dis stuct in the current Info folder as well as in the
% program folder. We need to figure out when to load the Info content.
...
% extraFolderName='SimpleRSPicker_Kv_dat/';  %%%%%%
% extraFolderName='SimpleRSPicker_KvAlpha_dat/';  %%%%%%
...
noFile=~(exist(dis.infoPath) && exist(dis.infoName));
if (dis.finished || newDis || ~dis.miValid || noFile) % We need to open a new file
...
%             end;
        ...
        case 'z' % load picker parameters from the current info directory
            [~,datBasename]=fileparts(datName);
            infoPath=fileparts(dis.infoName);
            localDatName=[infoPath datBasename '.mat'];
            if exist(datName,'file')>0
                disp(['Loading parameters: ' localDatName]);
                load(localDatName);
            else
                disp(['Parameter file not found: ' localDatName]);
            end;
...
save(datName,'dis');
[~,datBasename]=fileparts(datName);
infoPath=AddSlash(fileparts(dis.infoName));
save([infoPath datBasename '.mat'], "dis"); % also store the struct in our local directory.
if isa('si','struct')
