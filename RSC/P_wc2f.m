function [P_ws,dW2,Flag]=P_wc2f(alphas,betas,d_a,d_b,sigmaB,sigmaR,m,r,y,sigmaC)
...
%** angles as degrees?
%** separate this into 2 functions, use runInfo structure and par structure
%** xPars.sigmaC, sigmaB, sigmaR
%** (1) [logPmax ctr var]=rsEstimatePw(ri,xPars,r,y,io)  % computes the nalpha x
%** nbeta array of logPmax values.  Also returns nalpha x nbeta arrays of the 
%** intermediate values ctr and var for use by pWNorm.
%** Note that the result will be symmetric
%** in beta because of upper/lower hemispheres.
%** (2) pWNorm=rsComputePw(y,r,m,ctr,var)  % computes the m x m image p(w):
%** normalized probability, with w=0 in the center.
%** p(w|Z) is then given by exp(logPmax)*pWNorm
...
% define the center and std for the gauss distribution
    ...
    if abs(beta-pi/2)<8*pi/18
        ...
        for j=1:numel(alphas)
           ...
           % dCT2=rot
%**you could make ctr=[dx dy], sigma2=[sigma_X2 sigma_Y2] 2-element vectors and simplify lines.
% e.g. offs=sigmaC2*dCT+(m+1)*var./(sigmaC2+var);
%      var=sigmaC2*sigma2./(sigmaC2+sigma2);
%      logPmax (=Residual) =(-offs.^2)./var+dCT.^2./var+2*(m+1)^2/sigmaC2;
% **  Where do the factors (m+1) come from??
