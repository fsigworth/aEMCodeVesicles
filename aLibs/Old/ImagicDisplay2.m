function ImagicDisplay2(ImageStack, scale, contrastFraction, ShowLabels, LabelOffset)
% function ImagicDisplay2(ImageStack,scale, contrastFraction, ShowLabels, LabelOffset)
...
% is raised such that it saturates with lowest and
% highest 1% of pixel values.
...
% get(src)
% get(get(src,'parent'))
% get(get(src,'parent'),'parent')
% get(get(get(src,'parent'),'parent'),'parent')  % Position (x,y,w,h), CurrentPoint(x,yFromBottom)
end

function NewLocationFcn(loc)
        disp(['New location' num2str(loc)]);
%         loc is x, y location at the top.
...
%     fsize=max(7,min(10,round(showboxsize/4)));  % font size
    ...
    [rangeMin,rangeMax]=Percentile(bigmatrix,contrastFraction);
