...
% Determine ISO and RSO based on comparison of Relion's psi and the
% picker's psi values.
% -- old version, requires ves_particles.star file.
% -- see the new version rlMatchVesiclePsis which gets the picker psis
% directly from the particles.star file.
...
pStarName='Class3D/job031/run_it025_data.star';
%pStarName='Extract/job047/particles.star'
% Load a vesicle-particle file from the entire dataset.
%vStarName='RSC_newa/ves_particles1_ves_join.star';
vStarName='RSC2ab/ves_particles2ab_v.star'
...
dists=NaN(nParticles,1); % Distance to closest match particle
...
%     end;
    ...
    iVes(iParts,1)=vesRange(iVesLocal);
    dists(iParts,1)=minDists;
 end;
disp('done.');
disp([num2str(sum(iVes==0)) ' vesicles not matched.']);
figure(2);
hist(dists,100);
xlabel('Particle min distances');
ylabel('Frequency');
...
%      vesPsi=mod(vesPsi,360);
    ...
    psiDiff=mod(-vesPsi-psis+90,360);
     ...
     while ok
         ...
         outStarName=['particles_' orString '_' clsString suffix '.star'];
