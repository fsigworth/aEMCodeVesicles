...
%inStarName='Refine3D/job019/particles_rso_t20_all.star'; % Subtracted stack!
%inStarName='Select/job115/particles.star';
inStarName='Class3D/job027/run_it020_data.star'
doReadReferences=1; % 0 in case we want to re-use the reference read in!

% alpha alone, 128 pixel ref particles
refVStarName='Extract/job023/particles.star';
%refVStarName='JoinStar/job080/join_particles.star';
%refUStarName='JoinStar/job106/join_particles.star';
refUStarName='Extract/job022/particles.star';
...
% selectClasses=[3 4]; % vector of class numbers, 0 to select all.
selectClasses=0;
...
%%
if selectClasses
    classString=['_class' sprintf('%d',selectClasses)];
else
    classString='';
end;
...
if selectClasses
    active=any(dOutU.rlnClassNumber==selectClasses,2);
else
    active='';
end;
activeCells={''; active};
