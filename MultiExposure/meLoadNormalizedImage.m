...
%
% The returned image is scaled like the images in the proc or proc_sm
% directories, nominally scaled to the range (0..1) but with the
% mi.imageMedian subtracted. We do this scaling here for raw images.
% 
...
% If a raw image is loaded we scale the image according to
% m=m0*mi.imageNormScale+mi.imageNormOffset, and then subtract mi.imageMedian.
% (If imageNormOffset doesn't exist, we assume it's zero.)
% This is the scaling that is performed in rlStarToMiFiles for output files.
...
if any(targetSize<mi.padImageSize) % Look for an existing small image *s.mrc,
    ...
    for iPath=1:numel(paths)
        ...
        if exist(name,'file')
            mIn=ReadEMFile(name);
            nIn=size(mIn);
            if all(nIn*maxScaleUp>=targetSize) %% it's big enough
                ok=true;
                break;
            end;
        end;
...
if ~ok % Try reading a raw-sized image
    ...
    % We then scale it to reflect fractional image intensity, subtract the median, and pad it.
    ...
    if exist(fullImageName,'file') % we found one.
        ...
        me=mean(m0(:));
        % Put in k3/MotionCor2 params if needed.
        if ~isfield(mi,'imageACScale') || ~isfield(mi,'imageDCScale')
            mi.imageACScale=8;
            mi.imageDCScale=1;
            if mi.camera==7
                mi.cpe=.23;
            end;
            mi.imageNormScale=1/(mi.cpe*mi.pixA^2*sum(mi.doses));
        end;
        m1=((m0-me)*mi.imageACScale+me*mi.imageDCScale)*mi.imageNormScale;
        med=median(m1(:));
        m2=m1-med;
        
        if noShift
            mIn=Cropx(m2,mi.padImageSize);
        else
            mIn=Crop(m2,mi.padImageSize);
        end;
        rawImg=true;
        ok=true;
    end;
